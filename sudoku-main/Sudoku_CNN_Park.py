# Kyubyong-style Pure CNN: 3x3 conv blocks x10, 512 channels
...
class SudokuDS(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if self.aug and np.random.rand()<0.9: puz, sol = self._permute(puz, sol)
        x = encode_input(puz)
...
# --- model: 3x3x10, C=512, no pooling ---
class ConvBlock(nn.Module):
    def __init__(self, ch):
        super().__init__()
        self.conv = nn.Conv2d(ch, ch, 3, padding=1, bias=False)
        self.norm = nn.GroupNorm(32, ch)  # 작은 배치 안정
        self.act  = nn.ReLU(inplace=True)
    def forward(self, x): return self.act(self.norm(self.conv(x)))

class SudokuCNN512x10(nn.Module):
    def __init__(self, ch=512, depth=10):
        super().__init__()
        self.stem = nn.Sequential(
            nn.Conv2d(10, ch, 3, padding=1, bias=False),
            nn.GroupNorm(32, ch), nn.ReLU(inplace=True)
        )
        self.blocks = nn.Sequential(*[ConvBlock(ch) for _ in range(depth-1)])  # 총 10개(= stem 1 + blocks 9)
        self.head = nn.Conv2d(ch, 9, 1)  # per-cell 9-class
    def forward(self, x):
        x = self.stem(x); x = self.blocks(x)
        return self.head(x)  # (B,9,9,9)

model = SudokuCNN512x10(CH, DEPTH).to(DEVICE)
